...
while True:
    if not show_sketch_only:
        ...
        # === 색상 변경 기능 복원 (이 부분이 누락되었습니다) ===
        if sketch_enabled:
            # 좌측 상단 100x100 영역을 색상 감지 ROI(관심 영역)로 지정
            roi_frame = frame[0:100, 0:100]
            roi_hsv = hsv[0:100, 0:100]
            cv2.rectangle(frame, (0, 0), (100, 100), (200, 200, 200), 2) # ROI 영역 표시

            # 미리 정의된 색상 범위를 순회하며 감지
            for lower, upper in color_ranges:
                mask = cv2.inRange(roi_hsv, lower, upper)
                # 마스크에서 흰색 픽셀(색상이 감지된 픽셀) 수가 500개 이상이면
                if cv2.countNonZero(mask) > 500:
                    # 해당 색상의 평균값을 계산하여 새로운 그리기 색으로 지정
