...
# (이전 함수 및 설정 코드는 모두 동일합니다. 여기서는 생략합니다)
# on_trackbar_change, create_color_trackbars, combine_layers 함수 등은 그대로 두세요.
...
while True:
    ...
    if not show_sketch_only:
        ...
        if sketch_mode_displayed and sketch_mode_activated_time:
            if (cv2.getTickCount() - sketch_mode_activated_time) / cv2.getTickFrequency() < 2.0:
